...
class MyEnv(gymnasium.Env):
    ...
    'train' :[
        ...
        ((0.68, 1.55), (0.31, 1.41), -2.4)
    ],
    'maze1' :[
        ((1.36, 0.46), (0.2, 0.1), 1.6)
    ],
    'maze2' :[
        ((1.85,1), (1.4,1.85), 3.15)
    ]}

    def __init__(self, supervisor: Supervisor, map):
        super(MyEnv, self).__init__()
        self.INITIAL_AND_FINAL_STATES = self.ALL_INITIAL_AND_FINAL_STATES[map]
    ...
    def reset(self, seed=None):
        ...
        # set robot position
        self.trans_field.setSFVec3f([self.initial_pos[0], self.initial_pos[1], 0.001])
    ...
    def get_current_reward(self, distance_from_goal_diference):
        ...
        if min_lidar_reading <.05:
            return -1

        if min_distance_index > (3*size)//4:
            min_distance_index -=  size//4

        ang_reward = abs((size//4) - min_distance_index) / (size//2) 

        wall_distance_reward = min(abs(min_lidar_reading - self.WALL_DISTANCE_GOAL),5)

        total_reward = wall_distance_reward + ang_reward
